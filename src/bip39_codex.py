...
class Bip39Codec:
    ...
    def word_to_idx(self, word: str) -> int:
        try:
            ...
            raise ValueError(f"'{word}' is not a valid BIP-39 word")
    ...
    def phrase_to_indices(self, phrase: str) -> list[int]:
        return [self.word_to_idx(w) for w in phrase.strip().split()]
    ...
    def phrase_to_binary(self, phrase: str) -> str:
        return self.indices_to_binary(self.phrase_to_indices(phrase))

    def binary_to_phrase(self, binary_str: str) -> str:
        return self.indices_to_phrase(self.binary_to_indices(binary_str))

    # --------------- BIP-39 checksum: entropy <-> full phrase ----
    ...
    def phrase_to_entropy(self, phrase: str) -> tuple[bytes, bool]:
        """
        BIP-39 mnemonic phrase -> (entropy_bytes, checksum_valid).
    ...
    def validate_phrase(self, phrase: str) -> bool:
        """Return True if the provided mnemonic phrase has a valid checksum.

        This is a convenience wrapper around `phrase_to_entropy`.
        """
        _, ok = self.phrase_to_entropy(phrase)
        return ok

    def verify_entropy_checksum(self, entropy_bytes: bytes) -> bool:
        """Return True when `entropy_bytes` round-trips to a phrase and back.

        This converts entropy -> phrase -> entropy and ensures the checksum
        and the recovered entropy match the original bytes.
        """
        phrase = self.entropy_to_phrase(entropy_bytes)
        recovered, ok = self.phrase_to_entropy(phrase)
        return ok and recovered == entropy_bytes
